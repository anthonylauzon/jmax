...
explay_bang(fts_object_t *o, fts_symbol_t s, int ac, const fts_atom_t *at, fts_atom_t *ret)
{
  ...
  if (! explode)
    {
      fts_post("explay: %s: can't find explode\n", this->explode_name);
      this->current = 0;
    }
  else if (! this->current)
    fts_post("explay: not playing\n");
  else if (this->serial != explode->serial)
    {
      fts_post("explay: %s: has been modified while playing\n", this->explode_name);
      this->current = 0;
    }
  else
    {
      ...
      else
	{
	  ...
	  if (! this->serial)
		...
		if (e->pit == n1 && e->vel == n2 && e->chan == n3)
	      ...
	      if (! e)
		{
		  fts_post("explay: startat: note not found\n");
		  this->current = 0;
		}
	      else if (e->next)
		{
...
explay_nth(fts_object_t *o, fts_symbol_t s, int ac, const fts_atom_t *at, fts_atom_t *ret)
{
  ...
  if (! explode)
    {
      fts_post("explay: %s: no such explode\n", this->explode_name);
      this->current = 0;
    }
  else
    {
