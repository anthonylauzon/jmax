...
sampbuf_get(fts_symbol_t name)
{
  ...
  if(fts_hash_table_get(&the_sampbuf_hashtable, name, &data))
...
{
  fts_atom_t data;
  return(fts_hash_table_get(&the_sampbuf_hashtable, name, &data));
}
...
sampbuf_add(fts_symbol_t name, sampbuf_t *buf)
{
  ...
  fts_hash_table_put(&the_sampbuf_hashtable, name, &data);
}
