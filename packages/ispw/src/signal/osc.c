  ...
  for (i = 0; i < n; i ++)
    {
      ...
      idx = ((long)ph & TAB_K2) >> TAB_FRACBITS;
      out[i] = (float)((float) tab[idx].value + (float) tab[idx].slope * (float) ((long)ph & (TAB_FRAC - 1)));
    }
}
  ...
  for (i = 0; i < n; i ++)
    {
      ...
      idx = ((long)ph & TAB_K2) >> TAB_FRACBITS;
      sig[i] = (float)((float) tab[idx].value + (float) tab[idx].slope * (float) ((long)ph & (TAB_FRAC - 1)));
    }
}
