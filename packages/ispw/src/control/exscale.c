...
exscale_compute_params(exscale_t *x)
{
  ...
  if(in_range == 0. || out_range == 0.)
    {
      post("error: exscale: high and low value can not be the same\n");
      return;
    }
...
exscale_init(fts_object_t *o, int winlet, fts_symbol_t is, int ac, const fts_atom_t *at)
{
  ...
  x->params[in_high] = fts_get_number_arg(ac, at, in_high, 127.);
  x->params[out_low] = fts_get_number_arg(ac, at, out_low, 0.);
  x->params[out_high] = fts_get_number_arg(ac, at, out_high, 1.);
...
exscale_config(void)
{
  fts_metaclass_create(fts_new_symbol("exscale"), exscale_instantiate, fts_always_equiv);
}
