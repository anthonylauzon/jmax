...
/****************************************************************/
/* funzione di scrittura di fts : vedi client_fifo_put di peter */
static void halaudioport_output( fts_word_t *argv)
{
  ...
  /* and set a flag */
  while (FIFO_WRITE_LEVEL(this->fifo) < fts_get_tick_size()) {
...
/*        post( "*** Running scheduler for 1 tick (%d bytes)\n", fts_get_tick_size()*2*sizeof( float)); */
  ...
  {    
    memcpy( outOutputData->mBuffers[0].mData, 
	    /*&(this->fifo->data[this->fifo->read_index])*/this->fifo->data + this->fifo->read_index, 
	    outOutputData->mBuffers[0].mDataByteSize);
  ...
  else
  {
    ...
    FIFO_INCR_READ_INDEX(this->fifo, rest);
  }
...
static void halaudioport_delete(fts_object_t *o, int winlet, fts_symbol_t s, int ac, const fts_atom_t *at)
{
  ...
  halaudio_fifo_delete(this->fifo);
}
