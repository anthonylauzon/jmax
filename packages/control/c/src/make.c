...
make_args(fts_object_t *o, int winlet, fts_symbol_t s, int ac, const fts_atom_t *at)
{
  ...
  fts_set_object_with_type(&a, obj, this->classname);
...
make_set_classname(fts_object_t *o, int winlet, fts_symbol_t s, int ac, const fts_atom_t *at)
{
  ...
  fts_class_t *class = fts_class_get_by_name(classname);
...
{ 
  ...
  if(this->class == 0)
    fts_object_set_error(o, "Class name argument (symbol) required");
}
