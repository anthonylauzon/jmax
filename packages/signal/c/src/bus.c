...
static fts_symbol_t recycle_tilda_symbol = 0;
...
access_init(fts_object_t *o, int winlet, fts_symbol_t is, int ac, const fts_atom_t *at)
{
  ...
  idx = (int *)ftl_data_get_ptr(this->index);
  *bus = NULL;
  *idx = 0;
...
throw_ftl(fts_word_t *a)
{
  ...
  if(index < bus->n_channels)
    write = buffer + index * n_tick;
  else
    write = buffer + (bus->n_channels - 1) * n_tick;    
...
catch_ftl(fts_word_t *a)
{
  ...
  if(index < bus->n_channels)
    read = buffer + index * n_tick;
  else
    read = buffer + (bus->n_channels - 1) * n_tick;
...
recycle_put(fts_object_t *o, int winlet, fts_symbol_t s, int ac, const fts_atom_t *at)
{
  ...
  if(!fts_dsp_is_input_null(dsp, 0))
  {
    ...
    fts_set_symbol(a + 2, fts_dsp_get_input_name(dsp, 0));
    fts_set_int(a + 3, n_tick);
    fts_dsp_add_function(recycle_tilda_symbol, 4, a);
  }
}

static void
recycle_ftl(fts_word_t *a)
{
  bus_t *bus = *((bus_t **)fts_word_get_pointer(a + 0));
  int index = *((int *)fts_word_get_pointer(a + 1));
  float *input = (float *)fts_word_get_pointer(a + 2);
  int n_tick = fts_word_get_int(a + 3);
  float *buffer = bus->buf[1 - bus->toggle];
  float *write;
  int i;
...
static void
recycle_instantiate(fts_class_t *cl)
{
  fts_class_init(cl, sizeof(access_t), recycle_init, access_delete);

  fts_class_message_varargs(cl, fts_s_put, recycle_put);
...
signal_bus_config(void)
{
  ...
  recycle_tilda_symbol = fts_new_symbol("recycle~");
  ...
  fts_dsp_declare_function(recycle_tilda_symbol, recycle_ftl);
