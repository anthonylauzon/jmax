  ...
  for(i=0; i<n_tick; i++)
    {
      ...
      fts_wrapper_frac_set(&frac, in[i] * (double)WAVE_TABLE_SIZE);
      index = fts_wrapper_get_int(&frac, WAVE_TABLE_BITS);
  ...
  for(i=0; i<n_tick; i++)
    {
      ...
      fts_wrapper_frac_set(&frac, sig[i] * (double)WAVE_TABLE_SIZE);
      index = fts_wrapper_get_int(&frac, WAVE_TABLE_BITS);
      f = fts_wrapper_frac_get_wrap(&frac);
