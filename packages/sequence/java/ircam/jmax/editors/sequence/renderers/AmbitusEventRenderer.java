...
public void render(Object obj, Graphics g, int state, GraphicContext theGc) 
{
  ...
  {		
	...
	if(cue != -1)
	{
		...
		if(cue > 9)
			cuex = gc.getGraphicDestination().getFontMetrics(SequencePanel.rulerFont).stringWidth(""+cue)+1;
...
public boolean contains(Object obj, int x, int y, GraphicContext theGc) 
{
	...
	if((cue > -1) && ((x >= evtx-1 && x <= evtx+1)) || (x >= evtx-CUE_WIDTH && x <= evtx+1 && y >= 2 && y <= 2+CUE_HEIGHT))
		return true;
