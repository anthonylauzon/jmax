...
static void
sequence_tuple_update(sequence_t *this)
{
  track_t *track = sequence_get_first_track(this);
  
  fts_tuple_set_size(this->tuple, 0);
...
static void
sequence_tuple_append(sequence_t *this, track_t *track)
{
  fts_atom_t a;
...
sequence_delete(fts_object_t *o, int winlet, fts_symbol_t s, int ac, const fts_atom_t *at)
{
  ...
  fts_object_destroy((fts_object_t *)this->tuple);
...
sequence_instantiate(fts_class_t *cl, int ac, const fts_atom_t *at)
{
  ...
  fts_method_define_varargs(cl, fts_SystemInlet, fts_s_upload_child, sequence_upload_child);
  fts_method_define_varargs(cl, fts_SystemInlet, fts_s_upload, sequence_upload);
