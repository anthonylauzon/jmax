...
#define BUF_SIZE_8BITS 8192
#define BUF_SIZE_16BITS 8192
...
char* 
mmio_loader_error(fts_audiofile_t* aufile)
{
  return mmio_loader_errbuf;
}

void 
mmio_loader_log(char* fmt, ...)
{
  va_list args; 
...
mmio_loader_open_read(fts_audiofile_t* aufile)
{
  ...
  if (wave_realloc(wave, 8192) != 0) {
    mmio_loader_log("Failed to create the internal buffer (%s)", file);
    goto error_recovery;
  }
  ...
  if (error != 0) {
    if (error == MMIOERR_CHUNKNOTFOUND) {
      mmio_loader_log("Couldn't find the format chunk (%s)", file);
      goto error_recovery;
    } else {
      mmio_loader_log("Read failed (%s)", file);
      goto error_recovery;
    }
  }

  if (chunk.cksize < (long) sizeof(PCMWAVEFORMAT)) {
    mmio_loader_log("Invalid format chunk size (%s)", file);
    goto error_recovery;
  }
  ...
  if (wave->format.wFormatTag != WAVE_FORMAT_PCM) {
    mmio_loader_log("Unsupported sample format (%s)", file);
    goto error_recovery;
  }
  ...
  if (wave->format.wFormatTag != WAVE_FORMAT_PCM) {
    mmio_loader_log("Wave file with unsupported sample format (%s)", file);
    goto error_recovery;    
  }
  ...
  case 8:
    fts_audiofile_set_format(aufile, fts_s_8bits);
    break;
  case 16:
    fts_audiofile_set_format(aufile, fts_s_16bits);
    break;
  default:
    mmio_loader_log("Wave file with unsupported sample width (width=%d bits) (%s)", wave->format.wBitsPerSample, file);
    goto error_recovery;    
  }
  ...
  if (pos < 0) {
    mmio_loader_log("Read to set the read position (%s)", file);
    goto error_recovery;
  }
...
mmio_loader_open_write(fts_audiofile_t* aufile)
{
  ...
  format = fts_audiofile_get_format(aufile);
  if (format == fts_s_8bits) {
    bytes_per_sample = 1;
  } else if (format == fts_s_16bits) {
    bytes_per_sample = 2;
  } else {
    mmio_loader_log("Only the 8bits and 16bits formats are supported");
    goto error_recovery;
  }
  ...
  if (mmioCreateChunk(wave->hmmio, &wave->riff_chunk, MMIO_CREATERIFF) != MMSYSERR_NOERROR) {
    mmio_loader_log("Failed to create the wave chunk (%s)", file);
    goto error_recovery;
  }
  ...
  if (mmioCreateChunk(wave->hmmio, &chunk, 0) != MMSYSERR_NOERROR) {
    mmio_loader_log("Failed to create the format chunk (%s)", file);
    goto error_recovery;
  }  
  if (mmioWrite(wave->hmmio, (HPSTR) &wave->format, sizeof(wave->format)) < sizeof(wave->format)) {
    mmio_loader_log("Failed to write the format chunk (%s)", file);
    goto error_recovery;
  }
  if (mmioAscend(wave->hmmio, &chunk, 0) != 0) {
    mmio_loader_log("Failed to ascend from the format chunk (%s)", file);
    goto error_recovery;
  }
  ...
  if (mmioCreateChunk(wave->hmmio, &wave->data_chunk, 0) != MMSYSERR_NOERROR) {
    mmio_loader_log("Failed to create the data chunk (%s)", file);
    goto error_recovery;
  }  
...
mmio_loader_read(fts_audiofile_t* aufile, float** buf, int nbuf, unsigned int buflen)
{
  ...
  if (wave == NULL) {
    ...
    return wave_read_8bits(wave, buf, nbuf, buflen);
  } else if (wave->format.wBitsPerSample == 16) {
    return wave_read_16bits(wave, buf, nbuf, buflen);
  } else {
    return -1;
  }
}
...
mmio_loader_write(fts_audiofile_t* aufile, float** buf, int nbuf, unsigned int buflen)
{
  ...
  if (wave == NULL) {
    ...
    err = wave_write_8bits(wave, buf, nbuf, buflen);
  } else if (wave->format.wBitsPerSample == 16) {
    err = wave_write_16bits(wave, buf, nbuf, buflen);
  } else {
    return -1;
  }
...
int 
wave_read_8bits(wave_t* wave, float** buf, int nbuf, unsigned int buflen)
{
  float* fbuf;
  int n, nchan;
...
int 
wave_read_16bits(wave_t* wave, float** buf, int nbuf, unsigned int buflen)
{
  float* fbuf;
  short* sbuf;
  int n, nchan;
...
int 
wave_write_8bits(wave_t* wave, float** buf, int nbuf, unsigned int buflen)
{
  float* fbuf;
  int ch, nchan;
...
int 
wave_write_16bits(wave_t* wave, float** buf, int nbuf, unsigned int buflen)
{
  float* fbuf;
  short* sbuf;
  int ch, nchan;
