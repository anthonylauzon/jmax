...
typedef struct fts_sample_fifo
{
  ...
  int index; /* the read pointer (index of the next sample to read) */
  int fill; /* # of samples currently in the fifo */
...
#define fts_sample_fifo_get_fill(sf) ((sf)->fill)
#define fts_sample_fifo_get_space(sf)  ((sf)->size - (sf)->fill)

static fts_heap_t *sample_fifo_heap;

/* Create a new empty fifo, of a given size */
static fts_sample_fifo_t *
fts_sample_fifo_new(int size, int read_block, int write_block)
{
...
static void 
fts_sample_fifo_set_destroy_callback(fts_sample_fifo_t *sf, void (* fun)(void *), void *data)
{
...
*/
static void 
fts_sample_fifo_destroy(fts_sample_fifo_t *sf)
{
...
/* Ask for a pointer to a buffer of sample to be read; size of the buffer
   ...
   the buffer is locked until the call to _gotten (see below) 
*/
static int 
fts_sample_fifo_want_to_read(fts_sample_fifo_t *sf, fifo_sample_t **p)
{
  int ret;
  int fill;    
  ...
  fill = fts_sample_fifo_get_fill(sf);

  if(fill < sf->read_block)
    {
      if(sf->writer_eof)
	/* if the writer is gone just read the rest */
	ret = fill;
      else
	{
	  /* wait for full read block or writer eof */
...
/* Signal that the last get operation is completed, and the used buffer
   is empty; it may wake up a thread blocked in want_to_write if unlocking
   this buffer make the space available .

   Do not call with n == 0!
 */
static void 
fts_sample_fifo_have_read(fts_sample_fifo_t *sf, int n)
{
  if(n > 0)
    {
      pthread_mutex_lock(&(sf->mutex));
      
      sf->index = (sf->index + n) % sf->size;
      sf->fill -= n;
      
      /* unlock (potentionally) waiting writer when a full write block is free */
      if(n == sf->read_block)
	{
	  if(!sf->writer_eof && fts_sample_fifo_get_space(sf) >= sf->write_block)
...
fts_sample_fifo_reader_eof(fts_sample_fifo_t *sf)
{
  ...
  if(sf->writer_eof)
  ...
  else
    {
      ...
      /* unlock writer waiting for full block */
      pthread_cond_signal(&(sf->write_cond));
    }
...
static int 
fts_sample_fifo_want_to_write(fts_sample_fifo_t *sf, fifo_sample_t **p)
{
  if(sf->reader_eof)
    {
      /* if nobody reads anymore just give up */
      *p = 0;
      return 0;
    }
  else
    {
      ...
      space = fts_sample_fifo_get_space(sf);
      
      if(space < sf->write_block)
	{
	...
	ret = sf->write_block;
      ...
      *p = sf->buf + (sf->index + sf->fill) % sf->size;
...
/* Signal that the last put operation is completed, and the used buffer
   is full; it may wake up a thread blocked in want_to_read if unlocking
   this buffer make the samples available .
 */
static void 
fts_sample_fifo_have_written(fts_sample_fifo_t *sf, int n)
{
  if(n > 0)
    {
      pthread_mutex_lock(&(sf->mutex));
      
      /* increment fill status */
      sf->fill += n;

      /* unlock (possibly) waiting reader when a full read block is available */
      if(!sf->reader_eof && fts_sample_fifo_get_fill(sf) >= sf->read_block)
...
/* nobody will write anymore from the fifo;
   if the reader is waiting on a want_to_read, it will be
   ...
   */
static void 
fts_sample_fifo_writer_eof(fts_sample_fifo_t *sf)
{
  pthread_mutex_lock(&(sf->mutex));
...
/* Create a new empty fifo, of a given size */
static fts_cmd_fifo_t *
fts_cmd_fifo_new(void)
{
...
/* Destroy must be called when the two parties already agreed to give up
   ...
   will cause impredictable effects */
static void 
fts_cmd_fifo_destroy(fts_cmd_fifo_t *cf)
{
...
static void 
fts_cmd_fifo_exec_one(fts_cmd_fifo_t *cf)
{
  void (*fun)(void *);
  void *v;
...
static void 
fts_cmd_fifo_add_command(fts_cmd_fifo_t *cf,  void (*fun)(void *), void *v)
{
  pthread_mutex_lock(&(cf->mutex));
...
/*************************************************************
 *
 *  COMMAND THREAD (to execute commands)
 *
 */
...
static void 
fts_async_call(void (*fun)(void *), void *v)
{
...
/* The device data do not contain the AF file  descriptor, beacause it is a private data of the reader thread. */
struct readsf_data
{
  /* The reader thread */
...
/* Destroy callback */
void 
readsf_destroy_data(void *dev_data)
{
  fts_free(dev_data);
}
...
static fts_status_t 
pdtd_readsf_activate(fts_dev_t *dev)
{
  struct readsf_data *dev_data;
...
static fts_status_t 
pdtd_readsf_deactivate(fts_dev_t *dev)
{
  struct readsf_data *dev_data;
...
pdtd_readsf_get(fts_word_t *argv)
{
  ...
  struct readsf_data *dev_data = (dev)? fts_dev_get_device_data(dev): 0;
  int nchans = fts_word_get_long(argv + 1);
  int n = fts_word_get_long(argv + 2);
  
  if(dev && dev_data->active)
    {
      fifo_sample_t *in;
      int ret;
      int ch, i,j;
      
      ret = fts_sample_fifo_want_to_read(dev_data->fifo, &in);
      ...
      else
	{
      ...
      fts_sample_fifo_have_read(dev_data->fifo, ret);
    }
  else
    {
      int ch, i,j;
...
fts_readsf_worker(void *data)
{
  ...
  while(!eof)
    {
      int n_read, n_frames, n_write;
      fifo_sample_t *p;

      n_write = fts_sample_fifo_want_to_write(fifo, &p);
      n_frames = n_write / dev_data->nch;
      n_read = afReadFrames(file, AF_DEFAULT_TRACK, p, n_frames);

      fts_sample_fifo_have_written(fifo, n_read * dev_data->nch);

      /* end of file */
      if(n_read == 0 || n_read != n_frames)
	break;
    }
...
/* The device data do not contain the file  descriptor, beacause it is a private data of the writer thread. */
struct writesf_data
{
  /* The reader thread */
...
static fts_status_t 
pdtd_writesf_activate(fts_dev_t *dev)
{
  struct writesf_data *dev_data;
...
static fts_status_t 
pdtd_writesf_deactivate(fts_dev_t *dev)
{
  struct writesf_data *dev_data;
...
pdtd_writesf_put(fts_word_t *argv)
{
  ...
  if(dev)
    {
      struct writesf_data *dev_data = fts_dev_get_device_data(dev);
      ...
      if (dev_data->active)
	{
	  ...
	  ret = fts_sample_fifo_want_to_write(dev_data->fifo, &out);
...
#else
		  for (i=ch, j=0; j<n; i=i+nchans, j++)
		    out[i] = (short)(in[j] * 32767.0f);
#endif
		}

	      /* Unlock the buffer */
	      fts_sample_fifo_have_written(dev_data->fifo, ret);
	    }
  ...
  while(!eof)
    {
      ...
      n_read = fts_sample_fifo_want_to_read(fifo, &p);
      n_frames = n_read / dev_data->nch;
      n_write = afWriteFrames(file, AF_DEFAULT_TRACK, p, n_frames);

      fts_sample_fifo_have_read(fifo, n_write * dev_data->nch);

      /* if file is not fully written or writer didn't deliver a hole block it's time to say goodby */
      if(n_write != n_frames || n_read != dev_data->file_block)
	break;
    }
