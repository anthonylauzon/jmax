...
slider_send_properties(fts_object_t *o, int winlet, fts_symbol_t s, int ac, const fts_atom_t *at)
{
  ...
  fts_object_property_changed_urgent(o, fts_s_min_value);
  fts_object_property_changed_urgent(o, fts_s_max_value);
}
...
slider_config(void)
{
  fts_metaclass_create(fts_new_symbol("slider"),slider_instantiate, fts_always_equiv);
}
