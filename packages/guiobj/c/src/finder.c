...
static void fts_finder_find_friends( fts_object_t *o, int winlet, fts_symbol_t s, int ac, const fts_atom_t *at)
{
  ...
  if(fts_class_get_method_varargs( fts_object_get_class(target), fts_s_find_friends))
    fts_send_message_varargs(target, fts_s_find_friends, 1, a);
  else
    {
	fts_set_symbol(&a[1], fts_object_get_class_name(target));
	fts_send_message_varargs((fts_object_t *)fts_get_root_patcher(), fts_s_find, 2, a);
    }
}
