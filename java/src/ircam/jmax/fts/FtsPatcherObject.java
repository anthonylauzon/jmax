...
{  
  ...
  /** List of objects uploaded when Sketchpad is not created */
  
  private MaxVector unfinishedObjectList = new MaxVector();
  ...
  static public GraphicObject makeGraphicObjectFromServer(FtsServer server, FtsObject parent, int objId, String className, 
  ...
  private void endUpload()
  {
    ...
    if( getEditorFrame() != null)
    {
      ...
      /* update unfinished objects, add it the the sketchpad and clear the list */
      GraphicObject obj;
      for(Enumeration e = unfinishedObjectList.elements(); e.hasMoreElements();)
      {
        obj = (GraphicObject)e.nextElement();
        obj.setSketchPad(sketch);
        sketch.addNewObject(obj, false);
      }
      unfinishedObjectList.removeAllElements();
      /**************************************************************************/
  ...
  public void addObject(int nArgs , FtsAtom args[]) 
  {
    ...
    if(newObj.getSketchPad()!=null)
      newObj.getSketchPad().addNewObject(newObj, doEdit);
    else
      unfinishedObjectList.addElement(newObj);
