...
public class BeanPanel extends JPanel {
  ...
  private class PropertyUpdater {
      ...
      try
	{
	  if ( readMethod != null && readMethod.getParameterTypes().length == 0)
	    return readMethod.invoke( beanInstance, (Object[])null);
	}
    ...
    {
      if ( !isCustom)
	((JTextField)component).setText( getValueAsText());
    }
  ...
  public void display( Object beanInstance)
  {
	...
	addOne( beanInstance, descriptors[i], descriptors[i].getName());
  ...
  public void display( Object beanInstance, String propertyName, String labelName)
  {
    ...
    if ( i < descriptors.length)
      addOne( beanInstance, descriptors[i], labelName);
  ...
  public void update()
  {
    for ( Enumeration e = displayedProperties.elements(); e.hasMoreElements(); )
      ((PropertyUpdater)e.nextElement()).update();
  }
