...
public class ErmesObjExternal extends ErmesObjEditableObject {
  ...
  public boolean MouseDown_specific(Event evt,int x, int y) {
    if (!itsSketchPad.itsRunMode) {
      if(evt.clickCount>1) {
	if ((evt.modifiers & Event.CTRL_MASK) != 0) { //edit the field
  ...
  public void resize(int theH, int theV) {
    ...
    itsSketchPad.GetEditField().reshape(itsX+10/*-itsSketchPad.tx*/, itsY+1/*-itsSketchPad.ty*/, d.width,itsSketchPad.GetEditField().getFontMetrics(itsSketchPad.GetEditField().getFont()).getHeight()+20 );
