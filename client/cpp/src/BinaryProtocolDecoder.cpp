...
namespace client {
  ...
  Hashtable<const char*, const char*> BinaryProtocolDecoder::symbolTable;
...
#define qSymbolCache4 64
  ...
  void BinaryProtocolDecoder::endSymbolCacheAction( int input)
  {
    ...
    if ( !symbolTable.get( p, symbol))
      {
	symbol = std::strcpy( new char[std::strlen(p)+1], p);
  ...
  void BinaryProtocolDecoder::endObjectAction( int input)
  {
    ...
    FtsObject *obj = _serverConnection->getObject( (int)_lval);
  ...
  BinaryProtocolDecoder::BinaryProtocolDecoder( FtsServerConnection *serverConnection)
  {
