...
connection_check(fts_object_t *src, int woutlet, fts_object_t *dst, int winlet)
{
          ...
          fts_class_get_method_varargs(fts_object_get_class(dst), fts_s_propagate_input) == NULL)
  {
    ...
    return (fts_class_get_inlet_method(fts_object_get_class(dst), winlet, fts_dsp_signal_class) != NULL);
  }
  else
  {
    ...
    while( fts_iterator_has_more( &iter))
    {
      ...
      if(fts_is_pointer(&a))
      {
        ...
        /* outlets connects to everything */
        if(type == NULL)
          return 1;

        /* found inlet type match */
        if(fts_class_get_inlet_method(fts_object_get_class(dst), winlet, type) != NULL)
          return 1;
      }
    }
  }
...
fts_connection_new(fts_object_t *src, int woutlet, fts_object_t *dst, int winlet, fts_connection_type_t type)
{
  ...
  /* init cache */
  fts_message_cache_init(&conn->cache);
