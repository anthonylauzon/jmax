...
static void sgi_shmdac_init(void);
static void sgi_shmadc_init(void);
...
static void sgi_shmdac_init(void)
{
  ...
  fts_dev_class_set_open_fun( class, sgi_shm_open);
  fts_dev_class_set_close_fun( class, sgi_shm_close);

  fts_dev_class_sig_set_put_fun( class, sgi_shmdac_put);

  fts_dev_class_sig_set_get_nchans_fun( class, sgi_shm_get_nchans);
}
...
static void sgi_shmdac_put(fts_word_t *argv)
{
  ...
  for ( chan = 0; chan < nchans; chan++)
    {
      float *in = (float *) fts_word_get_ptr(argv + 3 + chan);
...
static void sgi_shmadc_init(void)
{
  ...
  fts_dev_class_set_open_fun( class, sgi_shm_open);
  fts_dev_class_set_close_fun( class, sgi_shm_close);
  fts_dev_class_sig_set_get_fun( class, sgi_shmadc_get);
  fts_dev_class_sig_set_get_nchans_fun( class, sgi_shm_get_nchans);
}
...
static void sgi_shmadc_get(fts_word_t *argv)
{
  ...
  for ( chan = 0; chan < nchans; chan++)
    {
      float *out = (float *) fts_word_get_ptr(argv + 3 + chan);
