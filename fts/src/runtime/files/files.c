...
int
fts_file_find(const char *name, const char *pathlist, char *pathname)
{
  char buf[1024];
  int fd;
  const char *p, *end;
  ...
  if (! pathlist)
    p = fts_symbol_name(fts_default_search_path);
  else
    p = pathlist;
  ...
  while (1)
	...
	return 0;	/* loop end, nothing found */
    }
}
...
fts_file_open(const char *name, fts_symbol_t dir, const char *mode)
{
  ...
  if (name[0] == '/')
	 ...
	 */
      ...
      if (fts_search_path)
	fd = fts_file_open_with_path(fts_search_path, dir, name, mode);

      if (fd >= 0)
	return fd;

      if (fts_default_search_path)
	fd = fts_file_open_with_path(fts_default_search_path, dir, name, mode);
...
static fts_symbol_t default_directory;
...
/* note that the 0xff representation of EOF is local */
