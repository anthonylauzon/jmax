...
static void fts_atom_list_iterator_copy(fts_atom_list_iterator_t *iter, fts_atom_list_iterator_t *other);
...
static fts_atom_list_cell_t *fts_atom_list_cell_new( const fts_atom_t *atom )
{
...
#ifdef ATOM_LIST_PROFILE
  atom_list_cell_count++;
  atom_list_cell_total_size += sizeof( fts_atom_list_cell_t);
#endif
...
{
  fts_atom_list_cell_t *new_tail = fts_atom_list_cell_new( atom);
...
int fts_atom_list_is_subsequence( fts_atom_list_t *list, int ac, const fts_atom_t *at)
{
  fts_atom_list_iterator_t iterator, ii;
  ...
  fts_atom_list_iterator_init( &ii, list);
...
/*
  for( fts_alist_iter_init( &iter, alist); !fts_alist_iter_end( &iter); fts_alist_iter_next( &iter))
    {
      fts_atom_t *a = fts_alist_iter_current( &iter);
    }
*/
...
static void fts_atom_list_export_fun(fts_data_t *d)
{
  ...
  fts_atom_list_iterator_init( &iterator, this);
