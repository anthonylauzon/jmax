...
static fts_ffun_t *ffun_get_by_name( fts_symbol_t name)
{
  ...
  if ( fts_hash_table_get( &the_fts_ffun_hashtable, name, &a))
...
int fts_ffun_exists( fts_symbol_t name)
{
  ...
  return (fts_hash_table_get( &the_fts_ffun_hashtable, name, &a));
}
...
void fts_ffun_new( fts_symbol_t name, float (*function)(float))
{
  ...
  fts_set_ptr( &a, ffun);
  fts_hash_table_put( &the_fts_ffun_hashtable, name, &a);
}
