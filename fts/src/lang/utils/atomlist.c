...
fts_heap_t *atom_list_cell_heap = 0;
fts_heap_t *atom_list_heap = 0;


void
fts_atom_list_cell_init( fts_atom_list_cell *cell)
{
  int i;
...
fts_atom_list_cell_new( void )
{
  ...
  if (cell)
    fts_atom_list_cell_init( cell);

  return cell;
}

void
fts_atom_list_cell_destroy( fts_atom_list_cell *cell)
{
}

void
fts_atom_list_cell_free( fts_atom_list_cell *cell)
{
  fts_atom_list_cell_destroy( cell);
  fts_heap_free((char *) cell, atom_list_cell_heap);
}

void 
fts_atom_list_init( fts_atom_list_t *list)
{
  list->head = 0;
  list->tail = 0;
}
...
void
fts_atom_list_free( fts_atom_list_t *list)
{
...
static void 
fts_atom_list_append_one( fts_atom_list_t *list, const fts_atom_t *atom)
{
  fts_atom_list_cell *tail;
...
void 
fts_atom_list_append( fts_atom_list_t *list, int ac, const fts_atom_t *atom)
{
  int i;
...
void 
fts_atom_list_iterator_init( fts_atom_list_iterator_t *iter, const fts_atom_list_t *list)
{
  iter->cell = list->head;
...
fts_atom_list_iterator_new( const fts_atom_list_t *list)
{
  ...
  iter = (fts_atom_list_iterator_t *) fts_malloc( sizeof( fts_atom_list_iterator_t ));
  if (iter)
...
void
fts_atom_list_iterator_free(fts_atom_list_iterator_t *iter)
{
  fts_free((char *)iter);
}

void 
fts_atom_list_iterator_next( fts_atom_list_iterator_t *iter)
{
...
int 
fts_atom_list_iterator_end( const fts_atom_list_iterator_t *iter)
{
  return iter->cell == 0;
}

fts_atom_t *
fts_atom_list_iterator_current( const fts_atom_list_iterator_t *iter)
{
  return iter->atom;
}
