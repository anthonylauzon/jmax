				...
				fts_object_t *(* fun)(fts_patcher_t *patcher, int ac, const fts_atom_t *at))
{
  ...
  if (fts_hash_table_get(&fts_doctor_table, class_name, &data))
    {
  ...
  fts_set_ptr(&data, d);
  fts_hash_table_put(&fts_doctor_table, class_name, &data);
...
int fts_object_doctor_exists(fts_symbol_t class_name)
{
  ...
  return fts_hash_table_get(&fts_doctor_table, class_name, &data);
}
