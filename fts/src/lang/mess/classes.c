				    ...
				    fts_equiv_fun_t equiv_fun)
{
  ...
  if (fts_hash_table_get(&fts_metaclass_table, name, &data))
    {
  ...
  else
    {
      ...
      fts_hash_table_put(&fts_metaclass_table, name, &data);
    }
...
static void fts_metaclass_alias_realize( fts_symbol_t new_name, fts_symbol_t old_name)
{
  ...
  if ((fts_hash_table_get(&fts_metaclass_alias_table, new_name, &data)) ||
      (fts_hash_table_get(&fts_metaclass_table, new_name, &data)))
    return;			/* error: duplicated meta class */
  else
    {
      fts_set_symbol(&data, old_name);
      fts_hash_table_put(&fts_metaclass_alias_table, new_name, &data);
    }
}
...
static fts_symbol_t fts_metaclass_get_real_name(fts_symbol_t name)
{
  ...
  if (fts_hash_table_get(&fts_metaclass_alias_table, name, &data))
...
fts_metaclass_t *fts_metaclass_get_by_name(fts_symbol_t name)
{
  ...
  if (fts_hash_table_get(&fts_metaclass_table, fts_metaclass_get_real_name(name), &data))
