...
static void fts_template_register_filename(fts_symbol_t name, fts_symbol_t filename, enum template_mode mode)
{
  ...
  fts_hash_table_put(&template_table, name, &d);
...
static fts_template_t *fts_template_find(fts_symbol_t name)
{
  ...
  if (fts_hash_table_get(&template_table, name, &a))
...
static fts_template_t *fts_template_for_file(fts_symbol_t filename)
{
  ...
  for (; (!   fts_hash_table_iterator_end(&hit)); fts_hash_table_iterator_next(&hit))
    {
      fts_template_t *template = (fts_template_t *) fts_get_ptr(fts_hash_table_iterator_current_value(&hit));
