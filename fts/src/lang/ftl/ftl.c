...
static fts_hash_table_t ftl_functions_table;
...
static void ftl_module_init(void)
{
  fts_hash_table_init( &ftl_functions_table);
}
...
int ftl_program_add_signal( ftl_program_t *prog, fts_symbol_t name, int size)
{
  ...
  fts_hash_table_put( &(prog->symbol_table), name, &a);

  return 1;
}


void ftl_declare_function( fts_symbol_t name, ftl_wrapper_t wrapper)
{
  fts_atom_t a;
  ...
  if (fts_hash_table_get( &ftl_functions_table, name,  &data))
    {
  ...
  fts_hash_table_put( &ftl_functions_table, name, &a);
}
...
void ftl_program_destroy( ftl_program_t *prog)
{
  ...
  fts_hash_table_iterator_init( &iter, &(prog->symbol_table));
...
static fts_status_t ftl_state_machine( fts_atom_list_t *alist, state_fun_t fun, void *user_data)
{
  ...
  fts_atom_list_iterator_init( &iter, alist);
	  ...
	  for ( n = 0; n < info->ninputs; n++)
	    {
	      ...
	      fts_hash_table_get( &(prog->symbol_table), s, &data);
...
static int ftl_program_allocate_signals( ftl_program_t *prog)
{
  ...
  for( fts_hash_table_iterator_init( &iter, &(prog->symbol_table));
      ...
      fts_hash_table_iterator_next( &iter) )
    {
      ...
      m = (ftl_memory_declaration *)fts_get_ptr(fts_hash_table_iterator_current_value( &iter));
...
static fts_status_t compile_portable_state_fun( int state, int newstate, fts_atom_t *a, void *user_data)
{
  ...
  case ST_CALL_FUN:
    {
      ...
      fts_hash_table_get( &ftl_functions_table, fts_get_symbol(a), &data);
      fdecl =  (ftl_function_declaration *) fts_get_ptr(&data);
  ...
  case ST_CALL_ARGV:
	...
	fts_hash_table_get( &(info->prog->symbol_table), fts_get_symbol(a), &data);
	mdecl = (ftl_memory_declaration *)fts_get_ptr(&data);
...
static void ftl_post_functions_table( void)
{
  ...
  for( fts_hash_table_iterator_init( &it, &ftl_functions_table);
    ...
    {
      decl = (ftl_function_declaration *)fts_get_ptr(fts_hash_table_iterator_current_value(&it));
      s = fts_hash_table_iterator_current_key( &it);
...
void ftl_program_post_signals( const ftl_program_t *prog)
{
  ...
  for( fts_hash_table_iterator_init( &iter, &(prog->symbol_table));
      ...
      fts_hash_table_iterator_next( &iter) )
    {
      ...
      m = (ftl_memory_declaration *)fts_get_ptr(fts_hash_table_iterator_current_value( &iter));
      s = fts_hash_table_iterator_current_key( &iter);
...
void ftl_program_fprint_signals( FILE *f, const ftl_program_t *prog)
{
  ...
  fprintf( f,  "/* signals declarations */\n");
