...
static void grow_table( int id)
{
  ...
  data_table = fts_realloc( data_table, data_table_size * sizeof(fts_data_t *));

  for (i = old_size; i < data_table_size; i ++)
    data_table[i] = 0;
}

static void fts_data_init_table(void)
{
  int i;

  data_table_size = DATA_TABLE_INIT_SIZE;
  data_table = fts_malloc( data_table_size * sizeof(fts_data_t *));
