...
static int fts_symbol_get_operator( fts_symbol_t s);
static void fts_symbol_set_operator( fts_symbol_t s, int op);
static int fts_symbol_is_operator( fts_symbol_t s);
...
static fts_hashtable_t fts_oldexpression_fun_table;

/* Structure used to store  assignements in expression states */

struct fts_oldexpression_assignement
{
  fts_symbol_t name;
  fts_atom_t   value;
  struct fts_oldexpression_assignement *next;
};
...
struct fts_oldexpression_state
{
  ...
  fts_oldexpression_assignement_t *assignements;	/* properties expressed in the expression */
...
/* Error utility */
static int expression_error(fts_oldexpression_state_t *e, int err, const char *msg, const char *arg)
{
  e->msg = msg;
  e->err_arg = arg;
  e->ret = err;
...
static void op_stack_push(fts_oldexpression_state_t *e, int op, int op_type)
{
  e->op_stack_p++;
...
static void op_stack_pop(fts_oldexpression_state_t *e)
{
  e->op_stack_p--;
}

static int op_stack_is_empty(fts_oldexpression_state_t *e)
{
  return (e->op_stack_p == -1);
}

static int op_stack_deeper_than(fts_oldexpression_state_t *e, int n)
{
  return (e->op_stack_p >= (n - 1));
}

static void op_stack_set_empty(fts_oldexpression_state_t *e)
{
  e->op_stack_p = -1;
}

static int op_stack_top(fts_oldexpression_state_t *e)
{
  return e->op_stack[e->op_stack_p];
}

static int op_stack_top_p(fts_oldexpression_state_t *e)
{
  return e->op_stack_p;
}

static int op_stack_peek(fts_oldexpression_state_t *e, int i)
{
  return e->op_stack[e->op_stack_p - i];
}

static int op_type_stack_top(fts_oldexpression_state_t *e)
{
...
static void value_stack_push(fts_oldexpression_state_t *e, const fts_atom_t *value)
{
  e->value_stack_p++;
  e->value_stack[e->value_stack_p] = *value;
}

static void value_stack_pop(fts_oldexpression_state_t *e, int pos)
{
  e->value_stack_p -= pos;
}

static int value_stack_is_empty(fts_oldexpression_state_t *e)
{
  return (e->value_stack_p == -1);
}

static int value_stack_is_deep(fts_oldexpression_state_t *e, int n)
{
  return (e->value_stack_p == (n - 1));
}

static int value_stack_deeper_than(fts_oldexpression_state_t *e, int n)
{
  return (e->value_stack_p >= (n - 1));
}

static void value_stack_set_empty(fts_oldexpression_state_t *e)
{
  e->value_stack_p = -1;
}

static fts_atom_t *value_stack_top(fts_oldexpression_state_t *e)
{
  return &(e->value_stack[e->value_stack_p]);
}

static int value_stack_top_p(fts_oldexpression_state_t *e)
{
  return e->value_stack_p;
}

static fts_atom_t *value_stack_peek(fts_oldexpression_state_t *e, int i)
{
  return &(e->value_stack[e->value_stack_p - i]);
}

static const fts_atom_t *current_in(fts_oldexpression_state_t *e)
{
  return &(e->expr[e->in]);
}

static int next_in(fts_oldexpression_state_t *e)
{
  return e->in++; 
}

static int more_in(fts_oldexpression_state_t *e)
{
...
static fts_oldexpression_fun_t get_expression_fun(fts_symbol_t name)
{
  fts_atom_t data, k;

  fts_set_symbol( &k, name);
  if (fts_hashtable_get(&fts_oldexpression_fun_table, &k, &data))
    return (fts_oldexpression_fun_t) fts_get_pointer(&data);
  else
    return (fts_oldexpression_fun_t) 0;
}

void fts_oldexpression_declare_fun(fts_symbol_t name, fts_oldexpression_fun_t f)
{
  fts_atom_t k, v;

  fts_set_symbol( &k, name);
  fts_set_pointer(&v, f);
  fts_hashtable_put(&fts_oldexpression_fun_table, &k, &v);
}


static int fts_op_eval(fts_oldexpression_state_t *e);
static void fts_oldexpression_add_assignement(fts_oldexpression_state_t *e, fts_symbol_t name, fts_atom_t *value);
static void fts_oldexpression_add_var_ref(fts_oldexpression_state_t *e, fts_patcher_t *scope, fts_symbol_t name);
...
static fts_oldexpression_state_t *fts_oldexpression_state_new(fts_patcher_t *scope,
							int expr_size, const fts_atom_t *expr)
{
  fts_oldexpression_state_t *expr_state;

  expr_state = (fts_oldexpression_state_t *) fts_heap_alloc(expr_state_heap);
  ...
  expr_state->ret          = FTS_OLDEXPRESSION_OK;	
...
void fts_oldexpression_state_free(fts_oldexpression_state_t *e)
{
  fts_oldexpression_assignement_t *p;
  ...
  while (p)
    {
      fts_oldexpression_assignement_t *p2;
...
/* Macro to wrap around calls to fts_oldexpression_eval_one and fts_op_eval *only* */

#define TRY(call)             {int ret; ret = (call); if (ret != FTS_OLDEXPRESSION_OK)  return ret;}


static int fts_oldexpression_eval_one(fts_oldexpression_state_t *e)
{
  int op_stack_tos;
  int value_stack_tos;
  ...
  while (status != expression_end)
      ...
      if (status == waiting_arg)
	{
	  if (fts_is_closed_par(current_in(e)))
	    return expression_error(e, FTS_OLDEXPRESSION_SYNTAX_ERROR, "Unbalanced closed parentesis", 0);	
		...
		return expression_error(e, FTS_OLDEXPRESSION_SYNTAX_ERROR, 
		...
		return expression_error(e, FTS_OLDEXPRESSION_SYNTAX_ERROR, "Syntax error, value missing after quote", 0);
	    }
	      ...
	      while (more_in(e) && (! fts_is_closed_cpar(current_in(e))))
		{
		  ...
		  TRY(fts_oldexpression_eval_one(e));
	      ...
	      if (fts_is_open_par(current_in(e)))
		{
		  ...
		  if (! fts_is_closed_par(current_in(e)))
		    return expression_error(e, FTS_OLDEXPRESSION_SYNTAX_ERROR, "Unbalanced parentesis", 0);	
	  ...
	  if (fts_is_open_sqpar(current_in(e)))
	      ...
	      if (value_stack_top(e) && fts_is_symbol(value_stack_top(e)))
		{
		  ...
		  /* call the function */
		  {
			...
			if (ret != FTS_OLDEXPRESSION_OK)
			  return expression_error(e, ret, "Error in function %s",
						  name);
		      }
		    else
		      return expression_error(e, FTS_OLDEXPRESSION_UNDEFINED_FUNCTION,
...
static int fts_oldexpression_eval_simple(fts_oldexpression_state_t *e)
{
  int op_stack_tos;
  int value_stack_tos;
  enum {waiting_arg, waiting_op, expression_end} status; 
  ...
  while (status != expression_end)
      ...
      if (status == waiting_arg)
	{
	  ...
	  if (fts_is_closed_par(current_in(e)))
		...
		{
		  if (! more_in(e))
		    return expression_error(e, FTS_OLDEXPRESSION_SYNTAX_ERROR, "Syntax error in tuple constant", 0);
	  ...
	  if (fts_is_open_sqpar(current_in(e)))
	    {
	      ...
	      if (! fts_is_closed_sqpar(current_in(e)))
		return expression_error(e, FTS_OLDEXPRESSION_SYNTAX_ERROR, "Syntax error in Array access", 0);
...
static int fts_op_eval(fts_oldexpression_state_t *e)
{
  ...
  if (op_type == FTS_UNARY_OP_TYPE)
	  ...
	  if (fts_is_symbol(tos))
	    {
	      ...
	      if (value)
		{
		  fts_oldexpression_add_var_ref(e, e->scope, varname);

		  if (fts_is_void(value))
		    return expression_error(e, FTS_OLDEXPRESSION_UNDEFINED_VARIABLE, "Variable %s is undefined",
					    varname);
		  else
		    *tos = *value;
		}
	    ...
	    return expression_error(e, FTS_OLDEXPRESSION_SYNTAX_ERROR, "Syntax Error", 0);
	  break;

	case FTS_OP_PLUS:
	  /* NOP, really */
	  if (! (fts_is_int(tos) || fts_is_float(tos)))
	    return expression_error(e, FTS_OLDEXPRESSION_OP_TYPE_ERROR, "Type error for operator +", 0);
	  break;
	  ...
	  if (fts_is_symbol(tos) && fts_is_object(ptos) && fts_object_is_patcher(fts_get_object(ptos)))
	    {
	      ...
	      if (value)
		{
		  fts_oldexpression_add_var_ref(e, (fts_patcher_t *)obj, varname);
		...
		return expression_error(e, FTS_OLDEXPRESSION_UNDEFINED_VARIABLE, "Variable %s is not defined ",
					varname);
	    }
	  else
	    return expression_error(e, FTS_OLDEXPRESSION_OP_TYPE_ERROR, "Type error for operator .", 0);
	...
	case FTS_OP_ASSIGN:
	  if (fts_is_symbol(ptos))
	    {
	      fts_oldexpression_add_assignement(e, fts_get_symbol(ptos), tos);
	...
	case FTS_OP_ARRAY_REF:
	  if (fts_is_int(tos) && fts_is_tuple(ptos))
		...
		*value_stack_top(e) = *fts_tuple_get_element(tup, idx);
		...
		return expression_error(e, FTS_OLDEXPRESSION_ARRAY_ACCESS_ERROR, "Index out of bounds", 0);
	    }
	  else
	    return expression_error(e, FTS_OLDEXPRESSION_OP_TYPE_ERROR, "Type error for array access", 0);
...
int fts_oldexpression_get_result_count(fts_oldexpression_state_t *e)
{
  return e->count;
}

int fts_oldexpression_get_status(fts_oldexpression_state_t *e)
{
  return e->ret;
}

const char *fts_oldexpression_get_msg(fts_oldexpression_state_t *e)
{
  return e->msg;
}

const char *fts_oldexpression_get_err_arg(fts_oldexpression_state_t *e)
{
  return e->err_arg;
}
...
static void fts_oldexpression_add_assignement(fts_oldexpression_state_t *e, fts_symbol_t name, fts_atom_t *value)
{
  ...
  p = (fts_oldexpression_assignement_t *) fts_heap_alloc(expr_prop_heap);
...
int fts_oldexpression_map_to_assignements(fts_oldexpression_state_t *e, 
...
void fts_oldexpression_add_variables_user(fts_oldexpression_state_t *e, fts_object_t *obj)
{
  fts_expr_var_ref_t *p;
...
#ifdef EXPRESSION_TRACE_DEBUG
void fts_oldexpression_printf_assignements(fts_oldexpression_state_t *e)
{
  fts_oldexpression_assignement_t *p;
...
fts_oldexpression_state_t *fts_oldexpression_eval(fts_patcher_t *scope,
					    int expr_size, const fts_atom_t *expr,
					    int result_size, fts_atom_t *result)
{
  ...
  /* Calles of fts_oldexpression_eval should free the resulting structure */

  e = fts_oldexpression_state_new(scope, expr_size, expr);

  while ((e->count < result_size) && (e->in < expr_size))
    {
      e->ret = fts_oldexpression_eval_simple(e);

      if (e->ret == FTS_OLDEXPRESSION_OK)
	{
  ...
  fts_log( "\n");
  fts_oldexpression_printf_assignements(e);
#endif
...
static int get_array_element(int ac, const fts_atom_t *at, fts_atom_t *result)
{
  if ((ac == 4) && fts_is_int(at + 2) && fts_is_tuple(at + 1))
	...
	*result = at[3];
    ...
    return FTS_OLDEXPRESSION_SYNTAX_ERROR;
}
...
void fts_kernel_oldexpression_init(void)
{
  expr_prop_heap = fts_heap_new(sizeof(fts_oldexpression_assignement_t));
  expr_var_ref_heap = fts_heap_new(sizeof(fts_expr_var_ref_t));
  expr_state_heap = fts_heap_new(sizeof(fts_oldexpression_state_t));

  fts_hashtable_init(&fts_oldexpression_fun_table, 0, FTS_HASHTABLE_MEDIUM);

  /* function installation */
  fts_oldexpression_declare_fun(fts_new_symbol("unique"), unique);
